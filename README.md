...
**담당: EDA · 통계 검정 · 피처 엔지니어링** (`notebooks/jh_EDA_FINAL.ipynb`)

30,198명 · 이탈률 11%의 불균형 통신 고객 데이터에서, "점수 올리기"보다 **타당성 검증을 먼저** 수행하는 분석을 담당했습니다.

- **가설 기반 EDA + 통계 검정** — Cohen's d · Cliff's δ · Mann–Whitney U · 카이제곱 · VIF · Kaplan–Meier 생존분석으로 단순 상관이 아닌 효과크기·비모수 검정 기반 이탈 신호 도출
- **반직관적 인사이트** — "상담비율 = 불만밀도"가 최강 이탈 신호(p≈10⁻⁷), "오래 쓰면 충성" 통념은 기각 등 가설 채택/기각 스토리 정리
- **피처 엔지니어링** — 35개 파생 피처 설계 → `data/churn/featured/churn.csv` (30,198 × 36)
- **누수 진단 & 분석 엄밀성** — 완전 중복행 0건 · 피처–타깃 최대 상관 0.078 확인, 누수 위험 피처 제거(35→29), 성능 천장 실험적 검증

**기술 스택**: Python · pandas · scikit-learn · SciPy(통계 검정) · lifelines(생존분석) · Jupyter
---
...
| **핵심 모델** | **LightGBM**(확률 보정) · ROC-AUC **0.906** · 해지 탐지율(Recall) **77%** · PR-AUC **0.581** |
...
4개 모델을 **불균형 보정**(class_weight / scale_pos_weight) 후, 단일 분할이 아닌 **RepeatedStratifiedKFold(5×2) 교차검증**으로 비교했습니다(불균형 문제이므로 정확도 대신 **PR-AUC**를 선정 기준으로).
...
**최종 모델(LightGBM, isotonic 확률 보정)** — 홀드아웃 test(6,040명):

| ROC-AUC | PR-AUC | Recall | Precision | F1 | Brier |
|:---:|:---:|:---:|:---:|:---:|:---:|
| **0.906** | **0.581** | **0.771** | 0.476 | 0.589 | **0.060** |

> **핵심:** 이탈 방지는 미탐(놓친 이탈자) 비용이 더 크므로 **recall 가중(F2) 운영점**을 채택 — 실제 이탈자 **664명 중 512명(77%)을 사전 포착**. 확률 보정으로 Brier **0.081→0.060** 개선(예측 확률을 신뢰 가능).
...
<div align="center">
  ...
  <br/>
  <sub>모델 비교(CV mean±std) · ROC/PR 곡선 · 혼동행렬 · 확률 캘리브레이션(보정 전후) · SHAP 변수 기여도</sub>
</div>

### 🔎 분석 엄밀성 (검증 로직)

성능 수치를 신뢰할 수 있도록, "점수 올리기"보다 **타당성 검증**을 먼저 수행했습니다.

- **누수 진단** — 완전 중복행 **0건**, 피처-타깃 최대 상관 **0.078**. 0.90대 성능은 누수가 아니라 *다수 약신호의 상호작용*에서 유래함을 확인(과적합 갭 ROC 0.09).
- **누수 안전 피처** — 전수 데이터로 산출된 **전도적(transductive) 분위·플래그 피처 5종**과 **완전중복 피처(`cs_ratio`=`cs_per_100min`, r=1.0)** 를 제외(35→29). 제거 후 오히려 PR-AUC 소폭 상승.
- **천장 확인** — RandomizedSearch(40회)·앙상블·범주형 인코딩 모두 유의미한 개선 없음 → 현재 성능이 데이터의 실질 한계에 근접함을 *실험으로 입증*(과도한 정규화는 오히려 급락).
- **확률 보정** — isotonic 보정으로 예측 확률의 신뢰도(Brier) 확보 → 대시보드의 "이탈 확률 %"가 실제 빈도와 정합.
- **누수 없는 임계값** — test가 아닌 **dev 교차예측(OOF)** 에서 F2 기준 선택.
